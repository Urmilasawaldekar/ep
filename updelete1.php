...
if(!isset($_POST['upload'])){
 ...
 while($row = mysqli_fetch_assoc($result)) {?> 
   ...
   <div >
      <h1 style="margin-left:580px;color:#1d4052;margin-bottom:30px;margin-top:20px;font-size:35px;"> Update Property</h1>

      <form action="#" method="POST" enctype="multipart/form-data" style='padding-left:500px;'>
      <input type="hidden" name="email" id="email" required="" value="<?php echo $row["email"];?>">
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["owner_name"];?>">
         <br><br>
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["contact"];?>">
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["product_name"];?>"><br><br>
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["price"];?>"><br><br>

         <div style='width:120px;color:black;'>Property Status:</div>
         
         <select name="srproperty" id="srproperty" class="srproperty" required="">
                                    <option value="">---- Select ----</option>
                                    <option  value="<?php echo $row['property_status']; ?>" selected><?php echo $row["property_status"]; ?></option>
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["area"];?>">
         <br><br>
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["img"];?>"><br><br>
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;' value="<?php echo $row["city_name"];?>">
        ...
        box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);
        margin-bottom: 20px;
        opacity: 0.9;'><?php echo $row["p_details"];?></textarea>
         <div>
           <center>  <button class="fbtn type="submit" name="upload" >update</button></center>
         </div>
...
//VALUES ('','$pname','$price','$address','$srproperty','$type','$psubtype','$message','$name','$area','$areasq','$oname','$contact','$email')";
 $update = "UPDATE `property` SET `product_name`='$pname',`price`='$price',`city_name`='$address',`property_status`='$srproperty',`type`='$type',`sub_type`='$psubtype',`p_details`='$message',`img`='$name',`area`='$area',`mesurement`='$areasq',`owner_name`='$oname',`contact`='$contact',`email`='$email' WHERE id ='$id'";
        if (mysqli_query($conn, $update)) {
          ...
          echo"<script>
          alert('Property not update successfully');
